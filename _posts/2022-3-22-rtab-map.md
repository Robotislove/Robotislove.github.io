...
这样一个定位点 $\mathrm{L}_{\mathrm{t}}$ 就可以用签名 $\mathrm{z}_{\mathrm{t}}$ 和时间索引 $\mathrm{t}$ 来创建, 其初始化的权重为 0 , 并与 $\mathrm{L}_{\mathrm{t}-1}$ 在 图中建立一个时间上的双向链接。
